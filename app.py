...
days_since_first_financing = st.number_input('Days Since First Financing', min_value=0)
total_raised = st.number_input('Total Raised', min_value=0.0, format="%.2f")
last_financing_size = st.number_input('Last Financing Size', min_value=0.0, format="%.2f")
competitor_count = st.number_input('Competitor Count', min_value=0)
ownership_status_acquired_merged = st.selectbox('Ownership Status: Acquired/Merged (Operating Subsidiary)', [0, 1])
days_since_last_financing = st.number_input('Days Since Last Financing', min_value=0)
most_recent_employment_year = st.number_input('Most Recent Employment Year', min_value=0)
number_of_employees = st.number_input('# Employees', min_value=0)
third_most_recent_employment_year = st.number_input('Third Most Recent Employment Year', min_value=0)
